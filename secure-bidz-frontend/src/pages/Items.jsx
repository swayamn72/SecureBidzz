  ...
  return (
    <div className="max-w-3xl mx-auto mt-10">
      ...
      <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {items.map(item => (
          <li key={item.id} className="border rounded-lg p-4 bg-white shadow hover:shadow-md transition">
            <Link to={`/items/${item.id}`} className="text-lg font-semibold text-blue-600 hover:underline">
              {item.title}
            </Link>
